...
from params_quad_obs import vx_bound,vy_bound,yaw_bound,obstacles_list,target,gain_attraction,gain_repulsion,yaw_gain, n_moving_obstacle, n_obs, x_lim,y_lim
from utils_obs import obstacle_circe, lidar_scan, get_pairs_collision, generate_obstacles_xml, quat_rotate_inverse_array
...
# def potential_field_planner(position,orientation,target,scan):
#     F_nav = gain_attraction*(target - position)
#     for i in range(scan.shape[1]):
#         diff_robot_obs = position - np.array([np.cos(scan[0,i]), np.sin(scan[0,i])]) * scan[1,i]
#         dist = max(0,np.linalg.norm(diff_robot_obs)) 
#         F_nav += (gain_repulsion/(dist**4))*(diff_robot_obs)
#     F_nav = np.hstack([F_nav,0])
#     F_nav = quat_rotate_inverse_array(orientation,F_nav)[:2]
...
def run_single_simulation(config, actor_network, decimation=16, max_steps=50000, noise_std=1.0, warmup_time=1.0, seed=None):
    ...
    data.qpos[:] = np.array([0., 0., 0.38, 0.92388, 0., 0., 0.38268] + list(default_joint_angles))
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        while viewer.is_running():
            for step in range(max_steps):
                ...
                # obstacles_scan = lidar_scan(model,data,np.pi*2,360,'trunk',7)
                for obst in obstacles_list:
                    obstacles_pos.append(model.body(obst).pos[:2])
...
def run_single_simulation_no_viewer(config, actor_network, decimation=16, max_steps=50000, noise_std=1.0, warmup_time=1.0, seed=None):
...
if __name__ == "__main__":
    run_batch_simulations(n_episodes=2,  save_path="observation_datasets", noise_std=10.0)
