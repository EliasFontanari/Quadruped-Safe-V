...
from utils_nn import generate_pairs, augment_dataset, normalize_states, get_batches, custom_lr_schedule

import seaborn as sns

from params import alpha,log_training_target,log_learning, learning_rate, tau, batch_size, epochs

from params_quad_obs import x_lim,y_lim
...
data_pairs = np.load(f"observation_datasets/pairs_dataset_parallel.npy")  #3110
...
for i in range(n_sub_batch):
    pairs_partition.append(jnp.array(data_pairs[i*sub_batch_len:min(pairs_len ,(i+1)*sub_batch_len)] , dtype=jnp.float32))
...
model = ValueNetwork()
params = model.init(key, jnp.ones((1, state_size)))        # network weights and shapes initialized
...
init_value=learning_rate, boundaries_and_scales={200*steps_for_epoch: 1e-1, 1000*steps_for_epoch: 1e-1 })
...
if not(log_learning):
    ...
    def loss_fn(params, target_params, pairs):
        ...
        V_next = jax.lax.stop_gradient(model.apply(target_params, pairs[:,-(state_size+2):-2]))
    ...
    def loss_fn(params,target_params, pairs):
        ...
        V_next = jax.lax.stop_gradient(model.apply(target_params, pairs[:,-(state_size+2):-2]))
        indicators = 1.0 - pairs[:,-1]
        target = jnp.minimum(0.,indicators * ((1-pairs[:,-2])*V_next/target_reached + pairs[:,-2]) * target_reached + (1-indicators) * target_fail)
...
saving_path = f'data_learning/plots/{now}_alpha_{alpha}_lr_{learning_rate}_batch_{batch_size}'
...
with tqdm(range(epochs), desc="Epoch") as pbar:
    for epoch in pbar:
        ...
        if epoch % 100 == 0:
            ...
            #           Save Model
            # ====================================
            model_path = f'{saving_path}/NN_JAX_log_learning_{log_learning}_epoch_{epoch}'
