...
def extract_actor_state_dict(checkpoint):
    ...
    if any(key.startswith("a2c_network.") for key in state_dict.keys()):
        ...
        for key, value in state_dict.items():
            if key.startswith("a2c_network.") and not key.startswith(
                "a2c_network.critic"
            ):
...
def convert_actor_network(torch_actor):
    """
    ...
    for name, module in torch_actor.actor_mlp.named_children():
        if isinstance(module, torch.nn.Linear):
            ...
            params[f"Dense_{dense_idx}"] = {"kernel": weight, "bias": bias}
            dense_idx += 1
...
checkpoint = torch.load(params.policy_path, map_location=params.device)
...
state_dict = torch.load(params.policy_path, map_location=params.device)["model"]
actor_state_dict = {
    k.replace("a2c_network.", ""): v
    for k, v in state_dict.items()
    if k.startswith("a2c_network.actor_mlp")
    or k.startswith("a2c_network.mu")
    or k.startswith("running_mean_std.running_mean")
    or k.startswith("running_mean_std.running_var")
    or k.startswith("running_mean_std.count")
}
...
flax_variables = convert_actor_network(torch_actor)
...
print(
    "✓ Outputs match!"
    if np.allclose(torch_output, flax_output, atol=1e-5)
    else "✗ Outputs don't match"
)
...
def norm_obs_running_jax(x):
    x_norm = (x - torch_to_jax(torch_actor.running_mean_std.running_mean)) / np.sqrt(
        torch_to_jax(torch_actor.running_mean_std.running_var) + 1e-5
    )
...
if True:
    for k in range(100):
        ...
        print(
            f"At step {k} running_mean {torch_actor.running_mean_std.running_mean[:5]}\n"
        )
